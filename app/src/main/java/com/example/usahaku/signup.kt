...
class signup : AppCompatActivity() {
    ...
    private fun signUpUser() {
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(inputEmails.text.toString()).matches()) {
            inputEmails.error = "Please enter valid email"
            inputEmails.requestFocus()
            return
        }

        if (inputPasswords.text.toString().isEmpty()) {
            inputPasswords.error = "Please enter password"
            inputPasswords.requestFocus()
            return
        }

        auth.createUserWithEmailAndPassword(inputEmails.text.toString(), inputPasswords.text.toString())
