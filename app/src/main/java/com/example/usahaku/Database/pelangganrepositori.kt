...
class pelangganrepositori(application: Application) {
    ...
    fun deleteAllpelanggan() {
        val deleteAllpelangganAsyncTask = DeleteAllpelangganAsyncTask(pelanggandao).execute()
    }
    ...
    companion object {
        ...
        private class DeleteAllpelangganAsyncTask(pelanggandao: Pelanggandao) : AsyncTask<Unit, Unit, Unit>() {
            val pelanggandao = pelanggandao

            override fun doInBackground(vararg p0: Unit?) {
                pelanggandao.deleteAllpelanggan()
            }
        }
