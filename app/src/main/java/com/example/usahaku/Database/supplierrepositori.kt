...
class supplierrepositori(application: Application) {
    ...
    fun insert(supplier: supplier) {
        val insertsupplierAsyncTask =
            InsertsupplierAsyncTask(supplierdao).execute(supplier)
    }
    fun update(supplier: supplier) {
        val updatesupplierAsyncTask =
            UpdatesupplierAsyncTask(supplierdao).execute(supplier)
    }
    fun delete(supplier: supplier) {
        val deletesupplierAsyncTask =
            DeletesupplierAsyncTask(supplierdao).execute(supplier)
    }
    fun getAllsupplier(): LiveData<List<supplier>> {
        return allsupplier
    }
    companion object {
        private class InsertsupplierAsyncTask(supplierdao: supplierdao) :
            AsyncTask<supplier, Unit, Unit>() {
            val supplierdao = supplierdao
            override fun doInBackground(vararg p0: supplier?) {
                supplierdao.insert(p0[0]!!)
            }
        }
        private class UpdatesupplierAsyncTask(supplierdao: supplierdao) :
            AsyncTask<supplier, Unit, Unit>() {
            val supplierdao= supplierdao
        ...
        private class DeletesupplierAsyncTask(supplierdao: supplierdao) :
            AsyncTask<supplier, Unit, Unit>() {
            val supplierdao = supplierdao
